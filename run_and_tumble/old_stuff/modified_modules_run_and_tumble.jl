...
function plot_decay_time_evolution(decay_times)
    ...
    # Function to compute and plot the average density field over time
    function plot_average_density_field(density_field::Array{Array{Float64, 2}})
        using Plots

        # Compute the average density field over time
        num_time_steps = length(density_field)
        summed_density = reduce(+, density_field)
        avg_density = summed_density / num_time_steps

        # Plot the average density field
        heatmap(avg_density, title="Average Density Field Over Time", xlabel="X-axis", ylabel="Y-axis", color=:viridis)
    end
end
