...
usage() {
    cat <<'EOF'
Usage:
  bash cluster_scripts/init_managed_diffusive.sh --case <diffusive_1d_pmlr|single_origin_bond> [options]
...
Case options:
  diffusive_1d_pmlr:   --gamma <float> --potential_strength <float>
                       defaults: gamma=1, potential_strength=16
  single_origin_bond:  --force_strength <float> --ffr <float>
                       defaults: force_strength=1, ffr=1
...
case "${case_name}" in
    diffusive_1d_pmlr)
        ...
        default_arg1="${gamma}"
        default_arg2="${potential_strength}"
        ;;
    single_origin_bond)
        if (( L % 2 != 0 )); then
            echo "--L must be even so the center bond is [L/2, L/2+1]. Got L=${L}." >&2
            exit 1
        fi
        managed_require_nonnegative_float "force_strength" "${force_strength}"
        managed_require_nonnegative_float "ffr" "${ffr}"
        default_arg1="${force_strength}"
        default_arg2="${ffr}"
        ;;
esac

if [[ -z "${run_id}" ]]; then
    run_id="$(managed_default_run_id "${case_name}" "${L}" "${rho}" "${default_arg1}" "${default_arg2}")"
else
...
registry_file="$(managed_registry_file "${REPO_ROOT}" "${case_name}")"
...
if [[ ! -f "${spec_path}" || "${force_spec_update}" == "true" ]]; then
    if [[ "${case_name}" == "diffusive_1d_pmlr" ]]; then
        cat > "${spec_path}" <<EOF
schema_version: 1
run_id: "${run_id}"
...
potential_magnitude: $(managed_format_float "${potential_strength}")
...
force_strength: $(managed_format_float "${force_strength}")
ffr: $(managed_format_float "${ffr}")
forcing_bond_pairs:
  - [${center}, ${next_x}]
forcing_magnitudes: [$(managed_format_float "${force_strength}")]
ffrs: [$(managed_format_float "${ffr}")]
...
if [[ "${case_name}" == "diffusive_1d_pmlr" ]]; then
    ...
    registry_header="timestamp,run_id,mode,L,rho0,force_strength,ffr,n_sweeps,num_replicas,request_cpus,request_memory,run_root,submit_dir,log_dir,state_dir,config_path,save_tag_prefix,aggregate_root,cumulative_tag"
    registry_row="$(printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" \
        "$(managed_timestamp)" "${run_id}" "managed" "${L}" "$(managed_format_float "${rho}")" "$(managed_format_float "${force_strength}")" "$(managed_format_float "${ffr}")" \
        "${segment_sweeps}" "${target_replicas}" "1" "managed" "${run_root}" "${batch_root}" "${batch_root}" "${replica_root}" "${spec_path}" \
        "managed" "${aggregate_root}" "managed_current")"
fi
...
echo "Initialized managed ${case_label} run."
echo "  run_id=${run_id}"
