...
Source options:
  ...
  --source_tag_prefix <prefix> Prefix for source state id tags, e.g. w20260421-153000_r
...
Behavior:
  ...
  - Warmup -> production defaults to --initial_state, so statistics are reset.
  - Production -> production defaults to --continue, so statistics keep accumulating.
  - The aggregate child job always aggregates the newest raw production states.
    In continue mode it replaces aggregate_root/current with the newest aggregate;
    in initial_state mode it folds the batch into the cumulative aggregate.
...
read_source_prefix_from_root() {
    local run_root="$1"
    local run_info_path="${run_root}/run_info.txt"
    if [[ -f "${run_info_path}" ]]; then
        read_run_info_value "${run_info_path}" "save_tag_prefix"
    fi
}
...
case "${continuation_mode}" in
    auto|initial_state|continue)
        ;;
    *)
        echo "--continuation_mode must be auto, initial_state, or continue. Got '${continuation_mode}'."
        exit 1
        ;;
esac
...
if [[ -n "${source_run_id}" ]]; then
    ...
    if [[ "${resolved_source_kind}" == "warmup" ]]; then
        ...
        source_tag_prefix="${source_tag_prefix:-$(read_source_prefix_from_root "${warmup_root}")}"
        ...
        source_tag_prefix="${source_tag_prefix:-$(read_source_prefix_from_root "${production_root}")}"
...
resolved_continuation_mode="${continuation_mode}"
if [[ "${resolved_continuation_mode}" == "auto" ]]; then
    if [[ "${source_kind}" == "production" ]]; then
        resolved_continuation_mode="continue"
    else
        resolved_continuation_mode="initial_state"
    fi
fi

if [[ "${source_kind}" == "warmup" && "${resolved_continuation_mode}" == "continue" ]]; then
    echo "Warmup sources cannot use --continuation_mode continue; that would include warmup statistics."
    exit 1
fi

aggregate_merge_mode="accumulate"
if [[ "${resolved_continuation_mode}" == "continue" ]]; then
...
while IFS= read -r initial_state; do
    ...
    replica_state_arg_name="--initial_state"
    if [[ "${resolved_continuation_mode}" == "continue" ]]; then
        replica_state_arg_name="--continue"
    fi
    replica_runner_arguments="${RUNNER_SCRIPT} ${runtime_config} ${replica_state_arg_name} ${initial_state} --save_tag ${replica_tag}"
...
aggregate_arguments="${AGGREGATE_SCRIPT} --config ${runtime_config} --raw_state_dir ${raw_state_dir} --num_replicas ${num_replicas} --raw_tag_prefix ${save_tag_prefix} --aggregate_root ${aggregate_root} --batch_tag ${batch_tag} --cumulative_tag ${cumulative_tag} --archive_stamp ${archive_stamp} --merge_mode ${aggregate_merge_mode} --run_info ${run_info}"
...
continuation_mode=${resolved_continuation_mode}
aggregate_merge_mode=${aggregate_merge_mode}
...
echo "  continuation_mode=${resolved_continuation_mode}"
echo "  aggregate_merge_mode=${aggregate_merge_mode}"
