...
usage() {
    cat <<'EOF'
Usage:
  bash cluster_scripts/submit_active_objects_L32_rho100_d16_hard_refresh_hist.sh \
      --num_replicas <int> \
      --n_sweeps <int> \
      --kappa <value> \
      --ots <int> \
...
Behavior:
  - Fixed physical setup:
      L = 32, rho = 100, d = 16, hard_refresh
...
BASE_CONFIG="${REPO_ROOT}/configuration_files/active_objects_1d_two_objects_L32_rho100_d16_hard_refresh_k5e-6.yaml"
...
if [[ -z "${run_id}" ]]; then
    run_id="active_objects_1d_two_objects_L32_rho100_d16_hard_refresh_${kappa_token}_${ots_token}_nr${num_replicas}_hist_${timestamp}"
fi

tmp_config="$(mktemp "${TMPDIR:-/tmp}/active_objects_L32_rho100_d16_hr_${kappa_token}_${ots_token}.XXXXXX.yaml")"
trap 'rm -f "${tmp_config}"' EXIT

description="active_objects_1d_two_objects_L32_rho100_d16_hard_refresh_${kappa_token}"
