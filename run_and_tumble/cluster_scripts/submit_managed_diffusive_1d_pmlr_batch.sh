...
Selection priority:
  1. idle ready replicas (thermalized, no production statistics yet)
  2. idle production replicas with the lowest statistics_sweeps
  3. idle warmup replicas closest to warmup_threshold_sweeps
  4. new random warmup replicas until target_replica_count is reached
EOF
}
...
WORKER_SCRIPT="${SCRIPT_DIR}/run_managed_diffusive_1d_pmlr_replica.sh"
...
dag_file="${submit_dir}/managed_diffusive_1d_pmlr.dag"
...
(
    ...
    selected=0
    while IFS=, read -r _pri _metric rid phase elapsed stats latest source_tag_path source_tag claim status updated; do
        [[ -n "${rid}" ]] || continue
...
echo "Prepared managed diffusive 1D PmLr batch."
echo "  run_id=${run_id}"
