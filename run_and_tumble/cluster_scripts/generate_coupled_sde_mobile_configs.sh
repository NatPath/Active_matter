...
INITIAL_MAX_SEPARATION="${INITIAL_MAX_SEPARATION:-}"
...
for mu_obj in "${MU_VALUES[@]}"; do
    ...
    cfg="${CONFIG_ROOT}/mu_${token}.yaml"
    {
        ...
        echo "random_initial_objects: ${RANDOM_INITIAL_OBJECTS}"
        echo "initial_min_separation: ${INITIAL_MIN_SEPARATION}"
        if [[ -n "${INITIAL_MAX_SEPARATION}" ]]; then
            echo "initial_max_separation: ${INITIAL_MAX_SEPARATION}"
        fi
        echo ""
