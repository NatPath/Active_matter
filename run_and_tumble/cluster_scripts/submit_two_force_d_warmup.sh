...
SOURCE_CONFIG_DIR="${REPO_ROOT}/configuration_files/two_force_d_sweep/warmup"
DEFAULT_SUBMIT_DIR="${SCRIPT_DIR}/generated_submit/two_force_d_sweep/warmup"
DEFAULT_LOG_DIR="${REPO_ROOT}/condor_logs/two_force_d_sweep/warmup"
DEFAULT_STATE_DIR="${REPO_ROOT}/saved_states/two_force_d_sweep/warmup"
...
JOB_BATCH_NAME="${JOB_BATCH_NAME:-two_force_d_warmup}"
...
for d in "${D_VALUES[@]}"; do
    ...
    if [[ ! -f "${config_path}" ]]; then
        config_path="${REPO_ROOT}/two_force_d_sweep/warmup/d_${d}.yaml"
    fi
    if [[ ! -f "${config_path}" ]]; then
        config_path="${REPO_ROOT}/d_${d}.yaml"
    fi
    if [[ ! -f "${config_path}" ]]; then
        echo "Skipping d=${d}: could not find config file"
        continue
    fi
    ...
    if [[ -n "${CONTINUE_STATE_DIR}" ]]; then
        continue_state="$(latest_matching_state "${CONTINUE_STATE_DIR}" \
            "aggregated/two_force_d${d}_warmup_*.jld2" \
...
EOF
        ...
        aggregate_arguments="${AGGREGATE_SCRIPT} --run_id ${aggregate_run_id} --mode warmup --state_dir ${STATE_DIR} --config_dir ${CONFIG_DIR} --d_min ${d} --d_max ${d} --d_step 1 --num_files ${NUM_REPLICAS} --aggregated_subdir aggregated"
...
if [[ "${REPLICA_STRATEGY}" == "dag" && "${NUM_REPLICAS}" -gt 1 ]]; then
    ...
    dag_append_final_notification_node "${dag_file}" "${SUBMIT_DIR}" "${LOG_DIR}" "$(dirname "${SUBMIT_DIR}")" "${JOB_BATCH_NAME}" "two_force_d_warmup" "${REPO_ROOT}"
