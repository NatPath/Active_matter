...
usage() {
    cat <<'EOF'
Usage:
  bash cluster_scripts/status_managed_diffusive.sh --case <diffusive_1d_pmlr|single_origin_bond> --run_id <managed_run_id> [options]
...
echo "Managed ${case_label} status"
echo "  run_id=${run_id}"
...
declare -A ledger_gap_counts=()
declare -A checkpoint_gap_counts=()
total=0
...
while IFS=, read -r rid phase elapsed stats latest _source _source_tag _claim status _updated; do
    ...
    ledger_gap=$((elapsed - stats))
    ledger_gap_counts["${ledger_gap}"]=$(( ${ledger_gap_counts["${ledger_gap}"]:-0} + 1 ))
    ...
    checkpoint_gap=$((live_elapsed - live_stats))
    checkpoint_gap_counts["${checkpoint_gap}"]=$(( ${checkpoint_gap_counts["${checkpoint_gap}"]:-0} + 1 ))
...
echo
echo "ledger_warmup_gap_counts"
for key in "${!ledger_gap_counts[@]}"; do
    echo "  ${key}=${ledger_gap_counts[${key}]}"
done | sort -n

echo
echo "checkpoint_aware_warmup_gap_counts"
for key in "${!checkpoint_gap_counts[@]}"; do
    echo "  ${key}=${checkpoint_gap_counts[${key}]}"
done | sort -n
