...
Behavior:
  - Creates runs/fluctuating_force_sde/<run_id>/.
  - Runs one Condor node per replica.
  - Final DAG node aggregates online density-variance accumulators.
EOF
}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
RUNNER_SCRIPT="${SCRIPT_DIR}/run_fluctuating_force_sde_from_config.sh"
ANALYZE_SCRIPT="${SCRIPT_DIR}/analyze_fluctuating_force_sde.sh"
...
if [[ -z "${run_id}" ]]; then
    run_id="fluctuating_force_sde_${config_stem}_nr${num_replicas}_${timestamp}"
fi
...
run_root="${REPO_ROOT}/runs/fluctuating_force_sde/${run_id}"
if [[ -e "${run_root}" ]]; then
...
dag_file="${submit_dir}/fluctuating_force_sde.dag"
runtime_config="${runtime_config_dir}/$(basename "${config_path}")"
aggregate_submit_file="${submit_dir}/fluctuating_force_sde_aggregate.sub"
aggregate_output_file="${log_dir}/fluctuating_force_sde_aggregate.out"
aggregate_error_file="${log_dir}/fluctuating_force_sde_aggregate.err"
aggregate_log_file="${log_dir}/fluctuating_force_sde_aggregate.log"
...
for ((replica_idx = 1; replica_idx <= num_replicas; replica_idx++)); do
    ...
    submit_file="${submit_dir}/sde_r${replica_idx}.sub"
    output_file="${log_dir}/sde_r${replica_idx}.out"
    error_file="${log_dir}/sde_r${replica_idx}.err"
    log_file="${log_dir}/sde_r${replica_idx}.log"
...
dag_append_post_notification_script "${dag_file}" "AGG" "${submit_dir}" "${log_dir}" "${run_root}" "${run_id}" "fluctuating_force_sde" "${REPO_ROOT}"

cat > "${run_info}" <<EOF
run_id=${run_id}
timestamp=${timestamp}
mode=fluctuating_force_sde
...
registry_file="${REPO_ROOT}/runs/fluctuating_force_sde/run_registry.csv"
mkdir -p "$(dirname "${registry_file}")"
if [[ ! -f "${registry_file}" ]]; then
    echo "timestamp,run_id,num_replicas,request_cpus,request_memory,run_root,state_dir,analysis_dir,config_path,aggregate_save_tag" > "${registry_file}"
fi
printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    "${timestamp}" "${run_id}" "${num_replicas}" "${request_cpus}" "${request_memory}" \
    "${run_root}" "${state_dir}" "${analysis_dir}" "${config_path}" "${aggregate_save_tag}" >> "${registry_file}"

echo "Prepared fluctuating-force SDE DAG:"
echo "  run_id=${run_id}"
