...
if [[ $# -lt 1 ]]; then
    echo "Usage: $0 <config_path> [extra run_fluctuating_force_sde args...]"
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "${SCRIPT_DIR}/../run_fluctuating_force_sde.jl" ]]; then
...
cd "${REPO_ROOT}"
"${JULIA_BIN}" --startup-file=no run_fluctuating_force_sde.jl --config "${CONFIG_PATH}" "${EXTRA_ARGS[@]}"
